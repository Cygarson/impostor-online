...
io.on("connection", (socket) => {
    ...
    socket.on("createRoom", (nickname, color, mode, callback) => {
        ...
        rooms[code] = { players: [], gameStarted: false, forcedMode: mode !== "random" ? mode : null, round: 0, scores: {}, word: "", votes: [], guessed: false };
        rooms[code].players.push({ id: socket.id, nickname, color });
        socket.join(code);
    ...
    socket.on("joinRoom", (code, nickname, color, callback) => {
        ...
        room.players.push({ id: socket.id, nickname, color });
        socket.join(code);
    ...
    socket.on("startGame", (code) => {
        ...
        const roles = {}; let impostors = [], kamikazeId = null;
        ...
        players.forEach(p => {
            ...
            if (sock) {
                sock.emit("yourRole", { knowsWord: knows, word: knows ? room.word : undefined, isKamikaze: isKam });
            }
        });
    ...
    socket.on("submitVote", (code, votedId) => {
        ...
        if (room.votes.length >= room.players.length) {
            const cnt = {};
            room.votes.forEach(i => cnt[i] = (cnt[i] || 0) + 1);
            const votedOut = Object.entries(cnt).sort((a, b) => b[1] - a[1])[0][0];
            const pl = room.players.find(p => p.id === votedOut);
            let msg = "";
            if (pl.id && pl.id === votedOut) {
                // find impostor/kamikaze?
                msg = pl.id ? (rooms[code].forcedMode === "kamikaze" && pl.id !== rooms[code].players[0].id ? "💣 Kamikaze wygrał!" : "✅ Impostor wykryty!") : "";
                rooms[code].scores[votedOut] = (rooms[code].scores[votedOut] || 0) + 1;
            }
            const summary = room.players.map(p => ({
                nickname: p.nickname, color: p.color, isImpostor: rooms[code].forcedMode === "kamikaze" ? p.id === room.players[0].id : !p.knowsWord, isKamikaze: p.id === rooms[code].players[0].id, score: room.scores[p.id] || 0
            }));
            io.to(code).emit("roundEnd", { message: msg, round: room.round, players: summary });
    ...
    socket.on("guessWord", (code, guess) => {
        ...
        if (correct) { room.scores[p.id] = (room.scores[p.id] || 0) + 1; }
        const summary = room.players.map(p => ({ nickname: p.nickname, color: p.color, isImpostor: !p.knowsWord, isKamikaze: false, score: room.scores[p.id] || 0 }));
        io.to(code).emit("roundEnd", { message: correct ? `${p.nickname} odgadł hasło!` : `${p.nickname} pomylił się.`, round: room.round, players: summary });
        room.gameStarted = false;
        room.votes = [];
        room.guessed = true;
    });
    ...
    socket.on("nextRound", (code) => {
        const room = rooms[code];
        if (room) { room.gameStarted = false; room.guessed = false; io.to(code).emit("playerList", room.players); }
    });
